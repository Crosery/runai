...
- **Intent memory + BM25 prefilter + LLM rerank** — Each `rnai_sess_*` keeps a bounded current-session short-memory queue (default 10, configurable in Settings; overflow drops the oldest item). BM25 runs on a compact intent summary built from the current prompt + cwd + agent CLI + that queue, then hands a configurable candidate set to the router LLM (default 30 candidates; DeepSeek v4-flash by default; any OpenAI-compatible / Anthropic / `claude-cli` backend works). Hybrid score = `BM25 × 0.4 + LLM_quality × 0.6`.
