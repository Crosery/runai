...
- **Intent recognition + BM25 prefilter + LLM rerank** — Each `rnai_sess_*` keeps a bounded current-session short-memory queue (default 10, configurable in Settings; overflow drops the oldest item). Before BM25, runai extracts a compact true-intent summary from the current prompt + prior bounded memory + cwd + agent CLI, producing domain tags, include terms, and exclude terms. That summary drives BM25, then a configurable candidate set goes to the router LLM (default 30 candidates; DeepSeek v4-flash by default; any OpenAI-compatible / Anthropic / `claude-cli` backend works). Wrong-domain candidates such as KTV/vehicle WebView skills are gated out for ordinary Android emulator debugging before the LLM sees them; image-regeneration follow-ups with reference-image constraints are normalized before recall.
...
- **Two modes** — `EXCLUSIVE` is for alternatives or ambiguity, so the main agent may ask the user to choose. `COMPATIBLE` is for fixed complementary workflows ("整套调试链路" / "完整发版流程"): the hook output tells the agent to activate all listed skills by default, while still allowing one minimal question when required inputs, permissions, high-cost actions, or irreversible risk are missing. Same-session dedup so already-adopted skills don't get re-recommended.
