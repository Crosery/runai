...
- 管理员重置任意用户密码：服务端 `POST /api/admin/users/{user_id}/reset-password`(require_admin → validate → argon2 hash + 轮换 api_key_hash，新 key 不回传，admin 可重置自己) + 本机 CLI `runai admin reset-password <username> [--password <pw>]`(直接写本地 runai.db，省略 --password 走 rpassword 隐藏输入，是"忘记密码手改 SQL"的正规替代)。DB 侧新增 `Database::set_user_credentials`(原子重写 password_hash + api_key_hash)。tests: `admin_reset_password_e2e` (7 fn) / `cli_admin_reset_password_e2e` (3 fn)。dashboard 第三条入口：Admin tab 用户表每行加「重置密码」按钮(`web/js/12-admin-scope-skills.js::adminUserAction` `reset-password` 分支，window.prompt 收新密码，同 `19-admin-publish-review.js` reject 理由 prompt 的既有惯例，直连同一端点，不 disable 自身行)；登录 401 抗枚举文案同步收口在 `13-auth-bulk-prefs.js::submitAuth`(统一"用户名或密码错误"，不区分账号不存在/密码错)；`scripts/runai-client-install.{sh,ps1}` 登录失败分支补一句"联系 server 管理员用 runai admin reset-password 重置"。
