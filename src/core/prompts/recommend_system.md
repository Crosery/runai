...
**任务型请求**（用户要产出实际工作成果：写东西 / 调研 / 生成 / 分析 / 审查 / 重构等）倾向给出**至少 1 个最接近的可用 skill**，而不是空推。reasoning 里如实写"动作命中、领域不专"即可（例："用户要深度调研 RL，deep-research 覆盖调研动作，虽非 RL 专用但可用"）。
...
1. **用户真实意图**一句话：要做什么？（如果末尾是吐槽/元 prompt / 纯系统消息 → 空 EXCLUSIVE；如果是要产出工作成果的任务 → 继续，倾向给至少 1 个）
