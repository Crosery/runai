...
**任务型请求**（用户要产出实际工作成果：写东西 / 调研 / 生成 / 分析 / 审查 / 重构等）：如果有动作形态命中的候选，给出**至多 1 个**最接近的可用 skill；**没有任何动作命中的候选仍然空推**，不要为了"给一个"而硬塞。reasoning 里如实写"动作命中、领域不专"即可（例："用户要深度调研 RL，deep-research 覆盖调研动作，虽非 RL 专用但可用"）。这是硬上限：宁可空推也不越过 1 个凑数，也不把弱相关候选拉进来。
