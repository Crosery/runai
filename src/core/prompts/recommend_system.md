...
准入：只选择 task / triggers / inputs / outputs 直接覆盖任务，且 not-for 不排除当前场景的候选。BM25、used、llm、adopt、feedback、group 只用于相关候选排序，不能单独构成准入。

模式：一个直接命中或互为替代用 exclusive；多个候选分别承担任务中明示的必要子任务时用 compatible。只选最小充分集合，不凑数。

空集：闲聊、元讨论、纯系统消息、没有动作命中、或 not-for 排除全部候选时，selected 返回空数组。
