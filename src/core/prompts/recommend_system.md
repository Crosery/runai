...
`[score:NN]` 标签是该 skill 的综合质量分（0-100），由 LLM 质量评估（40%）+ 用户实际使用打分（60%）合成。**这是相关性相同时的强 tiebreaker**：
- score ≥ 70：高质量、用户认可，相关时优先推
- score 40-70：中性，正常考虑
- score < 40：质量低或被用户负反馈，**相关也尽量不推**，除非 prompt 高度独占匹配
- 没有 `[score:NN]` 标签的 skill 表示尚未评估，按描述本身判断，不作惩罚
