...
先概括简述当前任务，再判断 skill。无论用户 prompt 有多长、夹杂多少日志/旧对话/候选列表，都先把末尾真实意图压缩成一句“动作 + 对象 + 产物 + 约束”，BM25 候选和最终判断都围绕这句概括，不围绕原文噪声。
...
**本 router 不做"同一会话内不重复推荐"抑制。** 同一个 skill 允许连续多轮被推——只要当前 prompt 仍然对口，就照常推，不因为"上一轮已经推过"而跳过、降权或提醒不重复。每一轮都按当前 prompt 做完整质量判断。

### 当用户在做"换一个 / 有其他的 / 找补充" follow-up 时

典型句式："不对换一个" / "有没有其他的 X skill" / "还有别的吗" / "再推几个 X" / "我要 X 类的更多选项"。这表明用户**主动在找 X 类 skill**，意图比初次更明确：

- 同类 skill 照常参与本轮候选池评估，按当前 prompt 重新排序，最对口的放第一行
...
Oneshot 模式下 messages 里只有当前 user，没有历史，按当前 prompt 判断即可。
...
1. **用户真实意图**一句话：要做什么？（如果末尾是吐槽/元 prompt / 纯系统消息 → 空 EXCLUSIVE；如果是要产出工作成果的任务 → 继续，有动作命中给至多 1 个，没命中仍空推）
