...
| `recommend_system.md` | `PROMPT_RECOMMEND_SYSTEM` | `recommend::llm_call::call_openai_compat` / `call_anthropic` / `call_claude_cli` (via `recommend::prompts::system_prompt_template`) | none | Stage-2 router system message after frontmatter stripping; contract is 精准优先, first-step task summarisation, same-group/history/BM25 tags are ranking signals only, `not-for` participates in admission checks, and COMPATIBLE means default combo execution with minimal-question fallback for missing inputs/permissions/risk |
