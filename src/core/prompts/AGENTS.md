...
| `recommend_system.md` | `PROMPT_RECOMMEND_SYSTEM` | `recommend::llm_call::call_openai_compat` / `call_anthropic` / `call_claude_cli` (via `recommend::prompts::system_prompt_template`) | none | Stage-2 router system message after frontmatter stripping; contract is 精准优先 with action-form-match admission (a covered *action* is a direct hit even when the domain is broad; **task-type prompts give at most 1 usable skill when an action-form-match exists, else empty** — hard cap), first-step task summarisation, same-group/history/BM25 tags plus behavioural `[adopt:NN%]`/`[fb:+P/-N]` markers are ranking signals only, `not-for` participates in admission checks, and COMPATIBLE means default combo execution with minimal-question fallback for missing inputs/permissions/risk |
| `recommend_user.md` | `PROMPT_RECOMMEND_USER` | `recommend::router::recommend_for_user_with_client` | `{USER_PROMPT}` (×1), `{INTENT_SUMMARY}`, `{BM25_CANDIDATE_LIMIT}`, `{CWD_BLOCK}`, `{PROJECT_CONTEXT_BLOCK}`, `{HISTORY_BLOCK}`, `{CANDIDATE_LISTING}`, `{TOP_K}` | Stage-2 user message sent to router LLM; the current prompt appears exactly once (truncated); BM25 candidates are recalled from the Stage-1 compact intent summary while the current prompt stays highest priority; quantity rule is 最小充分集合, with `{TOP_K}` as a ceiling rather than a target |
