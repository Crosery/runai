...
|---|---|---|---|---|
| `recommend_intent.md` | `PROMPT_RECOMMEND_INTENT` | `recommend::router::recommend_for_user_with_client` (via `recommend::prompts::intent_prompt_template`) | `{USER_PROMPT}`, `{DETERMINISTIC_FALLBACK}`, `{CWD}`, `{CLIENT_KIND}`, `{SESSION_MEMORY}` | Stage-1 model prompt; compresses raw current prompt + prior compact memory into a short BM25 intent artifact, never chooses skills or emits mode tags |
| `recommend_system.md` | `PROMPT_RECOMMEND_SYSTEM` | `recommend::llm_call::call_openai_compat` / `call_anthropic` / `call_claude_cli` (via `recommend::prompts::system_prompt_template`) | none | Stage-2 router system message after frontmatter stripping; contract is 精准优先 with action-form-match admission (a covered *action* is a direct hit even when the domain is broad, task-type prompts lean to 1 usable skill over empty push), first-step task summarisation, same-group/history/BM25 tags plus behavioural `[adopt:NN%]`/`[fb:+P/-N]` markers are ranking signals only, `not-for` participates in admission checks, and COMPATIBLE means default combo execution with minimal-question fallback for missing inputs/permissions/risk |
| `recommend_user.md` | `PROMPT_RECOMMEND_USER` | `recommend::router::recommend_for_user_with_client` | `{USER_PROMPT}` (×2), `{INTENT_SUMMARY}`, `{BM25_CANDIDATE_LIMIT}`, `{CWD_BLOCK}`, `{PROJECT_CONTEXT_BLOCK}`, `{HISTORY_BLOCK}`, `{ALREADY_ROUTED_BLOCK}`, `{CANDIDATE_LISTING}`, `{TOP_K}` | Stage-2 user message sent to router LLM; BM25 candidates are recalled from the Stage-1 compact intent summary while the current prompt stays highest priority; quantity rule is 最小充分集合, with `{TOP_K}` as a ceiling rather than a target |
...
| `hook_output.md` | `PROMPT_HOOK_OUTPUT` | `recommend::hook_output::render_hook_output` | `{MODE}`, `{REASONING_BLOCK}`, `{CANDIDATES_BLOCK}`, `{ACTIVATION_DIRECTIVE}`, `{SKIP_REMINDER_BLOCK}`, `{SESSION_HISTORY_BLOCK}`, `{FEEDBACK_PROTOCOL_BLOCK}` | rendered text returned as the `UserPromptSubmit` hook stdout; COMPATIBLE defaults to activating all listed skills and only asks a minimal question for missing inputs/permissions/risk |
