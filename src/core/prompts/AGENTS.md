...
| `recommend_system.md` | `PROMPT_RECOMMEND_SYSTEM` | `recommend::llm_call::call_openai_compat` / `call_anthropic` / `call_claude_cli` (via `recommend::prompts::system_prompt_template`) | none | Stage-2 router system message after frontmatter stripping; **routes off the Stage-1 intent summary, not the raw prompt** (`你依据的是意图摘要` framing — the quoted-history / tail-extraction detection moved to Stage-1 since Stage-2 no longer sees the original text). Contract is 精准优先 with action-form-match admission (a covered *action* is a direct hit even when the domain is broad; the **action-only / domain-not-precise fallback gives at most 1** usable skill, else empty — this is NOT a global numeric cap: multiple candidates that each precisely hit a distinct sub-task/alternative still push more per the mode tree), same-group/history/BM25 tags plus behavioural `[adopt:NN%]`/`[fb:+P/-N]` markers are ranking signals only, `not-for` participates in admission checks, COMPATIBLE means default combo execution with minimal-question fallback for missing inputs/permissions/risk. **Also owns the 输出格式 + 候选数量（最小充分集合） rule blocks** that used to be re-sent per request in `recommend_user.md`; there is no numeric hard cap (`硬上限`) anywhere in the prompt — `RecommendConfig.top_k` stays a code-side ceiling only |
| `recommend_user.md` | `PROMPT_RECOMMEND_USER` | `recommend::router::recommend_for_user_with_client` | `{INTENT_SUMMARY}`, `{BM25_CANDIDATE_LIMIT}`, `{CWD_BLOCK}`, `{PROJECT_CONTEXT_BLOCK}`, `{HISTORY_BLOCK}`, `{CANDIDATE_LISTING}` | Stage-2 user message sent to router LLM; carries ONLY dynamic content and **no longer embeds the raw user prompt** — the router精排 off `{INTENT_SUMMARY}` (the Stage-1 condensed intent) + `{CANDIDATE_LISTING}` + optional cwd/history/project blocks. `{USER_PROMPT}` was removed (Stage-1 already read the original; re-sending it duplicated text, changed every turn breaking the prefix cache, and needed truncation). The static 输出格式 + 候选数量 rule blocks live in `recommend_system.md` (prefix-cache friendly); quantity is decided by the system prompt's 最小充分集合 rule |
