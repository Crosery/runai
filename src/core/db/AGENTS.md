...
- Users (v15+): `create_user` / `find_user_by_username` / `find_user_by_api_key_hash` / `find_user_by_id` / `list_users` / `set_user_admin` / `set_user_disabled` / `update_user_prefs` / `rotate_api_key` / `set_user_credentials` (atomic `password_hash` + `api_key_hash` rewrite — backs admin password reset) / `delete_user` (auth row only — owned resources/dirs go through `SkillManager::delete_user_cascade`) / `anonymize_router_events_for_user` (null out `router_events.user_id`). (`users.rs`)
