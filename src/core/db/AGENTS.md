...
- Schema migration notes: `trash_entries`; `users`, `user_skill_library`, `resources.owner_user_id`, `router_events.user_id`; `community_skills` (PK `(uploader_uid, name)`) for the team-mode community market; structured `resource_ai_summary` with PK `(owner_user_id, name)` plus `search_doc`, `router_card`, `source_hash`, `prompt_hash`, and `format_key`; `users.session_key_hash` (issue #35) for browser sessions independent from hook api_keys; and `usage_events` for activation/feedback idempotency. Owner-aware separation: `owner_user_id IS NULL` = public pool, `Some(uid)` = uid's private. Empty AI-summary `owner_user_id` means the public-pool summary; private rows use the uid so same-named public/private skills never share an index row.
