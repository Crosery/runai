...
| `schema.rs` | schema + ALL migrations (monolithic) | `init_schema` (v1–v24) |
...
- **Recommend intent memory is bounded by the caller's limit.** `append_router_intent_memory(session_id, user_id, client_kind, memory, limit)` stores at most one capped short-memory row per call, then trims that exact `(session_id, user_id, client_kind)` scope by dropping oldest rows beyond `limit`. `limit == 0`, empty `session_id`, or empty memory is a no-op. Do not merge scopes: Pi/Codex/Claude/OpenCode memory must not cross-contaminate even if they share a native session string.
