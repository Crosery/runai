...
- `enrich_skills(mgr, limit, mode, verbose, concurrency, only_names)` — generates bilingual AI summaries for skills via the configured LLM. Each summary is stored in `resource_ai_summary` (DB) alongside an `llm_score` (0-10) used by the hybrid BM25 prefilter. Summary feeds BM25 retriever + routing LLM, not user-facing text.
- `EnrichMode::{MissingOnly, Stale, Force}` controls which skills are picked up: `MissingOnly` = no summary yet; `Stale` = missing OR SKILL.md mtime newer than stored summary's `updated_at`; `Force` = every skill regardless of state.
- `only_names: Option<&[String]>` is the targeted-refresh path. When `Some(names)` the planner first filters skills to that set, then **overrides `mode` to `Force`** for the listed names — caller's signal is "this skill just changed on disk, regenerate it regardless of mtime/exists checks". Called by `runai install`, `runai market-install`, and `runai scan` to enrich just the freshly-added skills. Pass `None` for the full bulk pass.
- `parse_enrich_response` requires a `task:` line in the LLM output. When the LLM goes off-format (typically when very large SKILL.md confuses it into emitting router-style `EXCLUSIVE\n<name>` text) the parser returns an empty summary so the caller logs an error and DB stays untouched. Retry mechanism is just running enrich again — LLM non-determinism means most failures clear on the next pass.
- `reevaluate_skill(mgr, skill_name, feedback_note)` — single-skill re-enrich with explicit user feedback (positive or negative) folded into the prompt. Adjusts both `llm_score` and `summary` in light of the feedback so the routing signal evolves with usage. Triggered by `runai recommend feedback <skill> --note "..."` and the hook output's feedback protocol footer (main Claude calls it when the user reacts to a recommendation).
