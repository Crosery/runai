...
- **`registration.rs` dedupe-fallback is public-pool-only (C3, scan_findings.md).** `scan_managed_dir` / `scan_agents_dir` scan the PUBLIC pool, so their "already in DB?" fallback lookup uses `db.find_resource_by_name_for_user(Skill, name, None)` (`owner_user_id IS NULL`), NEVER `db.list_resources(None, None).find(name)`. The unfiltered variant scanned every row incl. all users' privates, so a public dir with no row matched a different user's private same-name row → the public dir was never adopted AND (if that private row's description was stale) its metadata got overwritten from the public dir's SKILL.md — an owner-pool-invariant violation in the 2026-04 dataloss root-cause region. The public-pool filter still matches github-sourced public rows (`github:owner/repo:name`) whose id the `local:`/`adopted:`/`github:` prefix probes miss. Regression gate: `tests/scanner_owner_dedupe_e2e.rs` (default + non-default `RUNE_DATA_DIR`, asserts the private row's `directory`/`description` and physical dir are untouched).
