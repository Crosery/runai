...
- **`hook_output.md`** — single unified template for everything the main Claude agent sees. `{MODE}`, `{REASONING_BLOCK}`, `{CANDIDATES_BLOCK}`, `{SESSION_ID}`, `{ACTIVATION_DIRECTIVE}`, `{SKIP_REMINDER_BLOCK}`, `{SESSION_HISTORY_BLOCK}`, `{FEEDBACK_PROTOCOL_BLOCK}` — `{SKIP_REMINDER_BLOCK}` is empty when `skip_reminder_enabled == false`, otherwise renders `cfg.skip_reminder_template` as its own paragraph between activation directive and trailing footer.

## AI summary enrichment
- `enrich_skills(mgr, limit, mode, verbose, concurrency, only_names)` — generates AI summaries in the configured `summary_lang` via the configured LLM. Each summary is stored in `resource_ai_summary` (DB) alongside an `llm_score` (0-10) used by the hybrid BM25 prefilter. Summary feeds BM25 retriever + routing LLM, not user-facing text. **Gated**: returns an empty report immediately (with a one-line stderr nudge) unless `cfg.summary_lang_confirmed` — never enrich in a language the user never picked.
