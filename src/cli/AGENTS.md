...
- `crate::server::{ensure_running, serve, EnsureStatus}` — `Server` + the no-subcommand auto-spawn.
...
## Tests
- No `cli`-local `#[cfg(test)]` tests (none existed in the monolith). CLI behavior is exercised indirectly via the integration suites (`safety_e2e`, `multiuser_owner_e2e`) and the `recommend get` integration tests, which drive the real `runai` binary.
