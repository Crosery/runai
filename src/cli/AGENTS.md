...
- Bare `runai recommend` hook calls resolve local identity from `$HOME/.runai-identity`. Missing file keeps anonymous compatibility. A valid `api_key` is hashed and mapped to a user, then routed through `recommend_for_user_with_client(..., Some(user_id), client_kind)` so dashboard user prefs apply. `client_kind` comes from hook JSON `client_kind` / `host_kind`, then `RUNAI_HOST_KIND`, then defaults to `claude`; it scopes `rnai_sess_*` derivation and recommend intent-memory queues so Codex/Pi/Claude/OpenCode do not cross-contaminate. Malformed, stale, or disabled identity fails closed: stderr gets `# runai recommend skipped: ...`, no router call runs, and no anonymous `router_events` row is written.
