...
## Tests
- `server.rs::tests` (`#[cfg(all(test, not(target_os = "windows")))]` + unix `HOME_LOCK` via `test_support`) — HOME env mocking is ineffective on Windows (`dirs::home_dir()` ignores env vars there), so running these on Windows operates on the REAL user profile — `sm_backup` created a real backup on the 2026-07-02 Windows CI runner before the gate was added. Covers: `tool_router_has_expected_tools` enumerating all 22 registered tools (the R4 macro-coupling regression gate), `sm_status` JSON shape, `sm_backups` exact-message + created-backup cases, `sm_groups` description preview, and `sm_search` no-results fallback.
