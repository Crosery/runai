...
- **Recommend intent memory is current-session only and client-scoped.** `router_intent_memory` stores normalized short-memory rows under `(rnai_sess_*, user_id, client_kind)` and trims by `UserPrefs.intent_memory_limit` (default 10) with drop-oldest semantics. `client_kind` comes from hook JSON / `RUNAI_HOST_KIND` and is part of both session-id scoping and the BM25 intent summary, so Pi/Codex/Claude/OpenCode do not share memory. Historical sessions are not backfilled into the router prompt; BM25 uses the compact current-session intent summary and defaults to `UserPrefs.bm25_candidate_limit = 30` candidates.
