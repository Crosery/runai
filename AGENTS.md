...
- **`/skills/bundle/{name}`**: gzipped tar of the whole resolved skill directory. Owner-aware via `resolve_skill_dir`. Pure-Rust (`tar` + `flate2`), no `tar` subprocess. Lets remote clients cache `SKILL.md + scripts/ + references/` locally so referenced files work without a per-file HTTP round trip. Client-side cache + hook integration is the next iteration; the server-side bundle is in place now.
...
`cargo test -- --test-threads=1` (and therefore `ci-local.sh`) runs the **physical e2e suites** on unix — `safety_e2e`, `multiuser_owner_e2e`, `cli_target_symmetry`, `mcp_canonical_e2e`, `mcp_stdio`, `install_fixture_e2e` — which are the real regression gate for the destructive-path and owner-isolation invariants. Treat a single failure there as a release blocker, not a flake.

**Test count varies by platform**: unix currently runs 413 lib tests (issue #27 added 19 functional tests to `core::db::tests` covering the full `router.rs`/`router_stats.rs` surface — 17 of which run and 2 of which are `#[ignore]`d REAL_BUG pins — and 20 to `core::linker::tests` covering `remove_link`/`is_our_symlink`/`detect_entry_type`/`adopt_to_managed`/`move_dir`/`copy_dir_recursive`) + 61 integration tests (3 anti_explore_e2e + 1 auth_uniform_error_e2e + 5 cli_target_symmetry + 6 community_market_e2e + 3 install_fixture_e2e + 4 install_script_e2e + 7 mcp_canonical_e2e + 1 mcp_stdio + 9 multiuser_owner_e2e + 4 prompts_multiuser_e2e + 3 rate_limit_e2e + 4 router_skill_lifecycle + 7 safety_e2e + 4 server_mode_e2e) = 474 active, plus 3 ignored (`install_test::test_real_install_minimax` manual network test; `core::db::tests::router_event_by_id_should_preserve_user_id_but_currently_drops_it` and `..._since_ordered_..._drops_it` pin the CORRECT behavior for the real `user_id`-dropping bug tracked in issue #33 — un-ignore only once that SELECT is fixed). Windows skips `manager::tests` and every integration suite that mocks HOME or relies on symlinks — the count is lower there. That's intentional — see Key constraints. PLANNING §2.3 items 4/5/6 added `anti_explore_e2e`, `auth_uniform_error_e2e`, and `rate_limit_e2e`; each spawns the real binary inside an isolated HOME and asserts on wire format.
