...
| core::prefs | [src/core/prefs.rs](src/core/prefs.rs) | inline `//!` | `UserPrefs` (per-user dashboard / recommend prefs, stored as JSON in `users.prefs_json`; includes prompt injection flags plus recommend context controls: intent memory on/off, queue limit, BM25 candidate count) |
...
| core::recommend | [src/core/recommend/mod.rs](src/core/recommend/mod.rs) | [src/core/recommend/AGENTS.md](src/core/recommend/AGENTS.md) | Opt-in LLM skill router for `UserPromptSubmit` hook; split into config / router / intent / enrich / lang_validation / prompts / llm_call / hook_output |
...
| core::skill_metrics | [src/core/skill_metrics.rs](src/core/skill_metrics.rs) | inline `//!` | Pure functions blending router telemetry + explicit feedback + enrich `llm_score` + usage counts into the five-axis skill feedback radar (adoption / precision / rating / quality / heat); no I/O, callers supply counts from `db::feedback` (schema v26). |
...
- **Frontend (web/*)**: account pill in topbar (login button → modal; logged-in shows `username (admin)` + logout `×`). Settings tab has "我的偏好" sections gated on login: recommendation scope (`allow_public_recommend`), prompt injection toggles, recommend context controls (`intent_memory_enabled`, `intent_memory_limit` default 10, `bm25_candidate_limit` default 30), and hook behavior. Skills tab has a scope bar (全部 / 我的库 / 仅公共) + bulk actions + quick-fill / import / clear buttons. All operations route through `api()` wrapper which uses `credentials: 'same-origin'` so the `runai_session` cookie minted on login carries automatically.
...
- **Hook activation session ids are runai-owned.** Host integrations may send native session keys (Claude/Codex hook `session_id`, Pi-derived session keys, or a pre-normalized `runai_session_id`), but router output must only print literal `rnai_sess_*` ids from `recommend::runai_session_id_from_native`. `/skills/use/{name}` rejects any non-empty `session_id` that is not `rnai_sess_*`, so adoption rows do not store host env vars or raw agent session strings. If no native session key exists, hook output omits `--session-id`.
- **Recommend routing is two-stage, current-session only, and client-scoped.** `router_intent_memory` stores Stage-1 compact intent rows under `(rnai_sess_*, user_id, client_kind)` and trims by `UserPrefs.intent_memory_limit` (default 10) with drop-oldest semantics. The router reads the existing compact queue first, calls the same recommend model with `recommend_intent.md` to compress the raw prompt into `intent_llm_output`, gates wrong-domain candidates, runs BM25 from that compact artifact, then calls the Stage-2 router over the retrieved candidates **using only the intent summary — the raw user prompt is not re-sent to Stage-2** (deliberate responsibility split: Stage-1 is the sole stage that reads the original text, so its intent quality is the only chance to understand it). Only the Stage-1 compact output is appended to memory; raw user prompts never enter the queue. Historical sessions are not backfilled. BM25 defaults to `UserPrefs.bm25_candidate_limit = 30` candidates. `not-for` must not be positive BM25 text, and `llm_score` / `usage_count` / `group` must not resurrect an unrelated candidate. Candidate re-ranking is a hybrid of BM25 + enrich `llm_score` + a behavioural feedback factor (real adoption rate + explicit thumbs, `bm25 * 0.35 + llm/10 * 0.45 + feedback * 0.20`; `RUNAI_FEEDBACK_DISABLED=1` reverts to the legacy `0.4/0.6`), and each Stage-2 candidate line carries `[adopt:NN%]` / `[fb:+P/-N]` markers for the router LLM. Host-injected harness envelopes (`<task-notification>` / `<agent-message` / `<teammate-message` / `<local-command-`) are pre-gated before either LLM wave and never write a `router_events` row, so they neither cost tokens nor pollute the adoption funnel. Each stage sends a FIXED system prompt (byte-identical every request → provider prefix cache) plus a lean user message whose dynamic fields appear exactly once — Stage-1: cwd / client / session_memory / current prompt (the only stage that embeds the raw prompt, truncated head+tail to `LLM_PROMPT_CHAR_CAP = 4000` chars); Stage-2: intent summary + candidate listing (no raw prompt); both calls run `temperature=0` with bounded `max_tokens`. **Session no-repeat suppression was removed** — the same skill may be recommended on consecutive turns, no `ALREADY_ROUTED` block or 已推参考池 recall block is injected, and `router_session_recommended_skills` is telemetry-only (`record_session_adoption` for the feedback radar is unaffected).
...
- **Login auto-start** is available via `runai server --install-autostart` (`core::autostart`). macOS writes `~/Library/LaunchAgents/cn.crosery.runai.plist` (with `ProcessType=Interactive` so launchd does not background-QoS-starve the user-facing dashboard after idle) + `launchctl load -w`; Linux writes `~/.config/systemd/user/runai.service` + `systemctl --user enable --now`. Both inject `RUNAI_NO_AUTOSPAWN=1` into the service env so a manual `runai` TUI run doesn't try to spawn a second copy. Windows path is unimplemented — the command prints Task Scheduler instructions.
