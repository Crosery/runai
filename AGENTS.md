...
- **Recommend intent memory is current-session only and client-scoped, and intent recognition runs before BM25.** `router_intent_memory` stores normalized short-memory rows under `(rnai_sess_*, user_id, client_kind)` and trims by `UserPrefs.intent_memory_limit` (default 10) with drop-oldest semantics. The router reads the existing queue first, runs `intent::recognize_intent(current_prompt, previous_memory, cwd, client_kind)`, gates wrong-domain candidates, then appends the current prompt to memory — so the current prompt does not appear twice in the same turn's `session_memory`. Historical sessions are not backfilled. BM25 uses the structured intent summary and defaults to `UserPrefs.bm25_candidate_limit = 30` candidates. `not-for` must not be positive BM25 text, and `llm_score` / `usage_count` / `group` must not resurrect an unrelated candidate.
