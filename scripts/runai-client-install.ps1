...
#     companion's activate / feedback / sync / flush subcommands + the
...
if ($DoHook) {
    ...
    $companionBody = @'
# Auto-generated by runai-client-install.ps1. Overwritten on reinstall.
# runai-client (PowerShell companion) — activate / feedback / sync / flush
...
function Invoke-Activate {
    ...
    if (-not $Refresh -and (Test-Path $skillMd)) {
        # use cache
    } else {
        $ok = Invoke-FetchFile -Skill $Skill -Rel 'SKILL.md' -Dest $skillMd
        if (-not $ok) {
            if (Test-Path $skillMd) { Write-RunaiWarn "fetch failed, using stale cache for $Skill" }
            else { Write-RunaiDie "unable to fetch SKILL.md and no warm cache" }
        }
    }
    if ($All) { Invoke-FetchBundle -Skill $Skill -DestDir (Join-Path $cacheDir 'files') | Out-Null }
    else { foreach ($rel in $Include) { Invoke-FetchFile -Skill $Skill -Rel $rel -Dest (Join-Path (Join-Path $cacheDir 'files') $rel) | Out-Null } }
...
function Invoke-Sync {
    ...
    foreach ($s in $Skills) {
        ...
        $fetched = Invoke-FetchFile -Skill $s -Rel 'SKILL.md' -Dest (Join-Path $dir 'SKILL.md')
        if (-not $fetched) { Write-Host "runai-client sync: skipping $s (server non-200 or unreachable)"; $skip++; continue }
        if ($All) { Invoke-FetchBundle -Skill $s -DestDir (Join-Path $dir 'files') | Out-Null }
        $ok++
    }
